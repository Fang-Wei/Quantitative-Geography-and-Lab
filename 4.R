...
#######################
## 3. MERGING SPATIAL FEATURES: gUnaryUnion()
...
#######################
## 4. Point-in-Polygon, Data Join and Area Calculations: Using poly.counts(); left_join(); poly.areas(); 
...
#######################
## 5. Distance Analysis: Using gDistance() and gWithinDistance()
...
eth.list<-c("White", "Black", "Native American", "Asian", "Other")

access.eth<-xtabs(ethnicity~access) 
colnames(access.eth) <- eth.list
#Stacked Bar Plot
barplot(access.eth, names.arg=eth.list, legend = rownames(access.eth), 
